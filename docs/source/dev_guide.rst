...
First activate the ``goldfinch`` Conda environment and install ``pytest``.
...
.. code-block:: console
  ...
  $ make clean
  $ make install
  $ conda list -n goldfinch --explicit > spec-file.txt
...
Make a new version of goldfinch in the following steps:
